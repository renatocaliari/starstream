...
Demonstrates how to use broadcast() in SSE handlers
    ...
    for todo in todos:
        items.append(
            Div(
                ...
                Span(todo["text"], cls="ml-2" if not todo["completed"] else "ml-2 line-through"),
                Button("×", data_on_click=f"/todos/{todo['id']}/delete", cls="ml-2 text-red-500"),
                cls="flex items-center p-2",
            )
        )
    return Div(*items, id="todo-list", cls="space-y-2")
...
def home():
    return Div(
        ...
        stream.get_stream_element("todos"),
        render_todo_list(),
        Form(
            Input(name="text", placeholder="Add todo...", cls="border p-2 rounded"),
...
def add_todo(text: str):
    ...
    stream.broadcast(elements(render_todo_list(), "#todo-list"), target="todos")
